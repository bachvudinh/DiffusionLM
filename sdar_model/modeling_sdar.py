...
# flash_attn imports - wrapped in try/except for non-CUDA environments
try:
    from flash_attn.ops.triton.layer_norm import rms_norm_fn as flash_rms_norm
    flash_rms_norm_available = True
except ImportError:
    flash_rms_norm_available = False
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if self.training:
            ...
            attn_weights = None
            # For GQA, repeat KV heads to match query heads for SDPA
            num_key_value_groups = self.num_attention_heads // self.num_key_value_heads
            key_states = repeat_kv(key_states, num_key_value_groups)
            value_states = repeat_kv(value_states, num_key_value_groups)

            # Determine if we're in decoding mode (single token with causal mask)
            # vs prefilling mode (block processing with custom mask)
            # For block diffusion, we always use the prefilling path since
            # we process blocks and need the custom block attention mask
            is_decoding = (query_states.shape[2] == 1) and attention_mask is not None and torch.all(attention_mask)

            if is_decoding:
                # Decoding case: single token, use SDPA with no mask
                query_states = query_states.transpose(1, 2)  # [b, 1, h, d] -> [b, h, 1, d]
                ...
                attn_output = rearrange(attn_output, 'b h l d -> b l (h d)')
            else:  # prefilling or block diffusion
                # Prefilling/Block diffusion case: expand mask to [batch, 1, seq, seq] for proper SDPA broadcasting
                if attention_mask is not None:
                    # attention_mask is [batch, seq, seq], need [batch, 1, seq, seq]
