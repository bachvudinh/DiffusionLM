"""MPS (Metal Performance Shaders) attention backend for vdllm.

This backend uses PyTorch's MPS backend for GPU-accelerated attention
on Apple Silicon GPUs via the Metal Performance Shaders framework.
...
======================
    ...
    │                    MPS format (contiguous)                  │
    └──────────────────────────────────────────────────────────────┘

Note: MPS has some limitations compared to CUDA:
  - Fewer optimized kernels available
  - Some operations may fall back to CPU
  - Performance may be lower than MLX on Apple Silicon
"""
...
class MPSAttentionBackend:
    """MPS/Metal implementation of attention for block diffusion.

    Uses PyTorch's MPS backend for GPU-accelerated computations
    on Apple Silicon (M1/M2/M3/M4) via Metal.
    ...
    def prefill_attention(
        ...
        Uses PyTorch's scaled_dot_product_attention with MPS backend,
        combined with staircase masking for block-local attention.
    ...
    def denoise_attention(
        ...
        Gathers cached KV pairs and computes attention with the new block.
        ...
        # Concatenate: (batch, seq_len, num_kv_heads, head_dim)
        # where seq_len = cached + new
    ...
    def reshape_and_cache(
        ...
        for i in range(total_tokens):
            ...
            block_idx = slot // k_cache.shape[1]  # block_size
